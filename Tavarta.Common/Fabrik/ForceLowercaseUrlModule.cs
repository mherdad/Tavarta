...
namespace Tavarta.Common.Fabrik
{
    ...
    public class ForceLowercaseUrlModule : HttpModuleBase
    {
        ...
        {
            if (context.Request.Url != null && (context.Request.Url.AbsolutePath.StartsWith("get", StringComparison.InvariantCultureIgnoreCase)
                                                || context.Request.Url.AbsolutePath.StartsWith("/get", StringComparison.InvariantCultureIgnoreCase)))
            {
                return;
            }

            if (context.Request.Url != null && (context.Request.Url.AbsolutePath.EndsWith(".axd", StringComparison.InvariantCultureIgnoreCase)
